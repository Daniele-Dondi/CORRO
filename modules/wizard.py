...
def StartWizard(window):
    ...
    WizardWindow.geometry('500x620+500+150')
    WizardWindow.grab_set()
    LoadConnFile('C:/Users/Daniele/Documents/GitHub/CORRO/test.conn')  
    print("culo",WhichSiringeIsConnectedTo("Reactant1: juy"))
    print("clo",GetAllSyringeInputs())
    print("clot",GetAllOutputsOfSyringe(2))

    def Close():
      MsgBox = tk.messagebox.askquestion ('Exit Wizard','Are you sure you want to exit Wizard?',icon = 'warning')
      if MsgBox == 'yes':  
       WizardWindow.destroy()
       
    WizardWindow.protocol("WM_DELETE_WINDOW", Close)
