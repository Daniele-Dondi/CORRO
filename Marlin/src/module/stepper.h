...
#define MIN_ISR_LOOP_CYCLES (ISR_E_STEPPER_CYCLES + ISR_MIXING_STEPPER_CYCLES + GANG_N(NON_E_AXES, ISR_X_STEPPER_CYCLES, + ISR_Y_STEPPER_CYCLES, + ISR_Z_STEPPER_CYCLES, + ISR_I_STEPPER_CYCLES, + ISR_J_STEPPER_CYCLES, + ISR_K_STEPPER_CYCLES))
...
class Stepper {
  ...
  public:
      ...
      #endif
      static uint32_t motor_current_setting[3]; // TODO: longer array if NON_E_AXES >= 4 ?
  ...
  public:
    ...
    static inline void set_position(const xyze_long_t &abce) {
      set_position(LIST_N(NON_E_AXES, abce.a, abce.b, abce.c, abce.i, abce.j, abce.k), abce.e);
    }
  ...
  private:
    ...
    FORCE_INLINE static void _set_position(const abce_long_t &spos) {
      _set_position(LIST_N(NON_E_AXES, spos.a, spos.b, spos.c, spos.i, spos.j, spos.k), spos.e);
    }
